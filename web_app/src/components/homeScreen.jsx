...
const HomeScreen = ({ messages, onSendMessage, sessionId, loading, setLoading, handleNewChat,handleLogout}) => {
                ...
                {messages.length === 0 ? (
                    ...
                    messages.map((item) => {
                        return item.sender === "user" ? (
                            <UserQuestion key={item.id} message={item.message} />
                        ) : (
                            <AiChatResponse key={item.id} item={item} />
                        );
                    })
